...
public:
  ...
  void set_type_cloneIMU() { _type = TypeType::Type_CloneIMU; }
   ...
   */
  void update(const Eigen::VectorXd &dx, bool do_tekf = false) override {
    ...
    // Update orientation
    if (do_tekf && _type == TypeType::Type_CloneIMU) {
      newX.block(0, 0, 4, 1) = ov_core::quat_multiply_tekf(dq, quat());
    } else {
