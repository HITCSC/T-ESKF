...
inline void legend(const std::map<std::string, std::string> &keywords = {}) {
  size_t ncol = 2;
  size_t fontsize = 8;
  double handletextpad = 0.3;
  double borderpad = 0.3;
  std::string edgecolor = "gray";
  PyObject *kwargs = PyDict_New();

  PyDict_SetItemString(kwargs, "ncol", PyLong_FromSize_t(ncol));
  PyDict_SetItemString(kwargs, "fontsize", PyLong_FromSize_t(fontsize));
  PyDict_SetItemString(kwargs, "handletextpad", PyFloat_FromDouble(handletextpad));
  PyDict_SetItemString(kwargs, "borderpad", PyFloat_FromDouble(borderpad));
  PyDict_SetItemString(kwargs, "edgecolor", PyString_FromString(edgecolor.c_str()));

  // PyObject *args = PyTuple_New(0);
  // PyTuple_SetItem(args, 0, detail::_interpreter::get().s_python_empty_tuple);
  // PyObject *res =
