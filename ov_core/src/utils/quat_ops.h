...
inline Eigen::Matrix<double, 4, 1> quat_multiply_tekf(const Eigen::Matrix<double, 4, 1> &q, const Eigen::Matrix<double, 4, 1> &p) {
  Eigen::Matrix<double, 4, 1> q_t;
  Eigen::Matrix<double, 4, 4> Qm;
  // create big L matrix
  Qm.block(0, 0, 3, 3) = q(3, 0) * Eigen::MatrixXd::Identity(3, 3) + skew_x(q.block(0, 0, 3, 1));
...
inline Eigen::Vector3d tekf_so3_update(const Eigen::Vector3d &d_theta, const Eigen::Vector3d &d_z, const Eigen::Vector3d &p,
                                       int tekf_method) {
  Eigen::Vector3d d_p;
  if (tekf_method == 0) {
    d_p = d_z + skew_x(d_theta) * p;
  } else if (tekf_method == 1) {
    Eigen::Matrix3d temp_Ro =
        (Eigen::Matrix3d::Identity() + skew_x(d_theta) + d_theta * d_theta.transpose()) / (1 + d_theta.transpose() * d_theta);
    d_p = temp_Ro * (p + d_z) - p;
  } else if (tekf_method == 2) {
    d_p = exp_so3(d_theta) * p + Jl_so3(d_theta) * d_z - p;
  } else if (tekf_method == 3) {
    d_p = exp_so3(d_theta) * p +  d_z - p;
  } 
  
  else {
    printf("tekf is not ready for ![tekf_method={0,1,2,3}]!\n ");
  }
  return d_p;
}
