...
public:
  ...
  Recorder(std::string filename) {
    ...
    if (boost::filesystem::exists(filename)) {
      ...
      ROS_WARN("File name : %s", filename.c_str());
  ...
  void callback_odometry(const nav_msgs::OdometryPtr &msg) {
    ...
    cov_record.clear();
    for (int i = 0; i < 36; i++) {
      cov_record.push_back(msg->pose.covariance.at(i));
    }
    has_covariance = true;
    write();
  }
...
protected:
  ...
  void write() {
    ...
    if (has_covariance) {
      ...
      for (int i = 0; i < 6; i++) {
        for (int j = i; j < 6; j++) {
          // std::cout<<i<<" "<<j<<" "<<cov_record.size()<<std::endl;
          outfile << " " << cov_record.at(6 * i + j);
        }
      }
      outfile << std::endl;
