...
public:
  ...
  static void init_with_yaw0(const Eigen::Vector3d &gravity_inI, Eigen::Matrix3d &R_GtoI) {
    ...
    double pitch = - std::asin(z_axis(0));
    double roll =  std::atan2(z_axis(1),z_axis(2));

    R_GtoI <<std::cos(pitch), 0, -std::sin(pitch),
            std::sin(pitch)*std::sin(roll),std::cos(roll) , std::cos(pitch)*std::sin(roll),
            std::sin(pitch)*std::cos(roll),-std::sin(roll),std::cos(pitch)*std::cos(roll);
