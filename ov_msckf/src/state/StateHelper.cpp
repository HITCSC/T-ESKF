...
void StateHelper::TEKFPropagation(std::shared_ptr<State> state, const Eigen::Vector3d &p0, const Eigen::Vector3d &v0,
                                  const Eigen::MatrixXd &Phi, const Eigen::MatrixXd &Q) {
  assert(Phi.cols() == 15);

  Eigen::Matrix<double, 15, 15> Tk1;
  Tk1.setIdentity();
  Tk1.block<3, 3>(3, 0) = skew_x(state->_imu->pos());
  Tk1.block<3, 3>(6, 0) = skew_x(state->_imu->vel());

  Eigen::Matrix<double, 15, 15> Tk0_inv;
  Tk0_inv.setIdentity();
  Tk0_inv.block<3, 3>(3, 0) = -skew_x(p0);
  Tk0_inv.block<3, 3>(6, 0) = -skew_x(v0);

  Eigen::Matrix<double, 15, 15> Phi_trans = Tk1 * Phi * Tk0_inv;

  if (state->_features_SLAM.size() == 0) {
    state->_Cov.topRows(15) = Phi_trans * state->_Cov.topRows(15);
    state->_Cov.leftCols(15) = state->_Cov.leftCols(15) * Phi_trans.transpose();
    state->_Cov.topLeftCorner(15, 15) += Tk1 * Q * Tk1.transpose();
    state->_Cov = state->_Cov.selfadjointView<Eigen::Upper>();
    return;
  }

  int pf_size = state->max_covariance_size() - 15;
  Eigen::MatrixXd Pf_trans;
  Eigen::MatrixXd Pf_s;
  Pf_trans = Eigen::MatrixXd::Zero(pf_size, 3);
  Pf_s = Eigen::MatrixXd::Zero(pf_size, 3);
  for (int i = 0; i < state->_variables.size(); i++) {
    if (state->_variables[i]->type() == TypeType::Type_Landmark) {
      auto landmark_i = std::dynamic_pointer_cast<Landmark>(state->_variables.at(i));
      Pf_trans.block<3, 3>(landmark_i->id() - 15, 0) = skew_x(landmark_i->get_xyz(0)) * Phi.block<3, 3>(0, 9);
      Pf_s.block<3, 3>(landmark_i->id() - 15, 0) = skew_x(landmark_i->get_xyz(0));
    }
  }


  // Pf_t = Pf_s * Phi.block<3, 3>(0, 9)

  //   P_k1
  // =
  //   Phi_t*P_11*Phi_t'                   & Phi_t*P_11*Pf_t' + Phi_t*P_12
  //   Pf_t*P_11*Phi_t' + P_21*Phi_t'      & Pf_t*P_11*Pf_t' + P_21*Pf_t' + Pf_t*P_12 + P_22
  // +
  //   Tk1*Q*Tk1'                          & Tk1*Q*Pf_s'
  //   Pf_s*Q*Tk1'                         & Pf_t*Q*Pf_s'

  Eigen::MatrixXd Phi_P11 = Phi_trans * state->_Cov.topLeftCorner(15, 15);
  Eigen::MatrixXd Phi_P12 = Phi_trans * state->_Cov.topRightCorner(15, pf_size);
  Eigen::MatrixXd Pf_P12 = Pf_trans * state->_Cov.block(9, 15, 3, pf_size);

  Eigen::MatrixXd Pf_P11_Pf_T = Pf_trans * (state->_Cov.topLeftCorner(15, 15)).block(9, 9, 3, 3) * Pf_trans.transpose();
  Eigen::MatrixXd Pf_Q_Pf_T = Pf_s * (Q).block(0, 0, 3, 3) * Pf_s.transpose();
  state->_Cov.topLeftCorner(15, 15) = Phi_P11 * Phi_trans.transpose() + Tk1 * Q * Tk1.transpose();
  state->_Cov.topRightCorner(15, pf_size) = Phi_P12 + (Phi_P11).block(0, 9, 15, 3) * Pf_trans.transpose() + (Tk1*Q).block(0,0,15,3)* Pf_s.transpose()  ;
  state->_Cov.bottomRightCorner(pf_size, pf_size) += Pf_P11_Pf_T + Pf_Q_Pf_T + Pf_P12 + Pf_P12.transpose();
  state->_Cov = state->_Cov.selfadjointView<Eigen::Upper>();
}
...
void StateHelper::EKFUpdate(std::shared_ptr<State> state, const std::vector<std::shared_ptr<Type>> &H_order, const Eigen::MatrixXd &H,
  ...
  Eigen::VectorXd dz = K * res;
  Eigen::VectorXd dx = dz;

  if (state->_options.do_tekf) {

    for (size_t i = 0; i < state->_variables.size(); i++) {
      if (state->_variables.at(i)->type() == TypeType::Type_IMU) {
        Eigen::Vector3d current_p = state->_imu->pose()->pos();
        Eigen::Vector3d current_v = state->_imu->vel();
        Eigen::Vector3d d_theta = dz.block<3, 1>(state->_variables.at(i)->id(), 0);
        Eigen::Vector3d d_zp = dz.block<3, 1>(state->_variables.at(i)->id() + 3, 0);
        Eigen::Vector3d d_zv = dz.block<3, 1>(state->_variables.at(i)->id() + 6, 0);
        // std::cout<<"current_p_id : "<< state->_variables.at(i)->id() + 3 << std::endl;

        dx.block<3, 1>(state->_variables.at(i)->id() + 3, 0) = tekf_so3_update(d_theta, d_zp, current_p, state->_options.tekf_method);
        dx.block<3, 1>(state->_variables.at(i)->id() + 6, 0) = tekf_so3_update(d_theta, d_zv, current_v, state->_options.tekf_method);
      }

      else if (state->_variables.at(i)->type() == TypeType::Type_CloneIMU) {
        auto clone_imu_i = std::dynamic_pointer_cast<PoseJPL>(state->_variables.at(i));
        Eigen::Vector3d clone_p = clone_imu_i->pos();
        Eigen::Vector3d d_theta = dz.block<3, 1>(state->_variables.at(i)->id(), 0);
        Eigen::Vector3d d_zp = dz.block<3, 1>(state->_variables.at(i)->id() + 3, 0);
        dx.block<3, 1>(state->_variables.at(i)->id() + 3, 0) = tekf_so3_update(d_theta, d_zp, clone_p, state->_options.tekf_method);
      }

      else if (state->_variables.at(i)->type() == TypeType::Type_Landmark) {
        // PRINT_ERROR(RED "tekf is not ready for [update:Type_Landmark]!\n" RESET);
        auto landmark_i = std::dynamic_pointer_cast<Landmark>(state->_variables.at(i));
        // double anchor_clone_timestamp = landmark_i->_anchor_clone_timestamp;

        Eigen::Vector3d d_theta = dz.block<3, 1>(0, 0);
        Eigen::Vector3d d_zp = dz.block<3, 1>(state->_variables.at(i)->id(), 0);
        if (landmark_i->_feat_representation == LandmarkRepresentation::Representation::GLOBAL_3D) {
          Eigen::Vector3d p_landmark = landmark_i->get_xyz(0);
          dx.block<3, 1>(state->_variables.at(i)->id(), 0) = tekf_so3_update(d_theta, d_zp, p_landmark, state->_options.tekf_method);
        } else {
          PRINT_ERROR(
              RED
              "TEKF is not ready for slam update with ![feat_representation==LandmarkRepresentation::Representation::GLOBAL_3D] !\n" RESET);
          assert(false);
        }

      } else {
        // PRINT_ERROR(RED "tekf is not ready for [???]!\n" RESET);
        ;
      }
    }
  }
  ...
  for (size_t i = 0; i < state->_variables.size(); i++) {
    bool do_tekf_so3_update = state->_options.do_tekf && (state->_variables.at(i)->type() == TypeType::Type_IMU ||
                                                          state->_variables.at(i)->type() == TypeType::Type_CloneIMU);
    state->_variables.at(i)->update(dx.block(state->_variables.at(i)->id(), 0, state->_variables.at(i)->size(), 1), do_tekf_so3_update);
  }
...
void StateHelper::augment_clone(std::shared_ptr<State> state, Eigen::Matrix<double, 3, 1> last_w) {
  ...
  if (state->_options.do_calib_camera_timeoffset) {
    ...
    if (state->_options.do_tekf) {
      Eigen::Matrix3d R_IinG = state->_imu->Rot().transpose();
      Eigen::Vector3d p_IinG = state->_imu->pos();
      dnc_dt.block(0, 0, 3, 1) = R_IinG * last_w;
      dnc_dt.block(3, 0, 3, 1) = skew_x(p_IinG) * R_IinG * last_w + state->_imu->vel();
    }
