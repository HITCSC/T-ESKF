...
struct StateOptions {
  ...
  void print(const std::shared_ptr<ov_core::YamlParser> &parser = nullptr) {
    if (parser != nullptr) {
      parser->parse_config("use_fej", do_fej,false);
      parser->parse_config("use_tekf", do_tekf,false);
      parser->parse_config("tekf_method",tekf_method,false);
      parser->parse_config("init_scale",init_scale,false);
    ...
    PRINT_DEBUG("  - use_tekf: %d\n", do_tekf);
    PRINT_DEBUG("  - tekf_mothod: %d\n", tekf_method);
