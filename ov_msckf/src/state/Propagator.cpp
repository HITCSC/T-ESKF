...
void Propagator::propagate_and_clone(std::shared_ptr<State> state, double timestamp) {
  ...
  double dt_summed = 0;
  Eigen::Vector3d imu_p0 = state->_imu->pos();
  Eigen::Vector3d imu_v0 = state->_imu->vel();
  ...
  if (state->_options.do_tekf) {
    StateHelper::TEKFPropagation(state, imu_p0, imu_v0, Phi_summed, Qd_summed);
  } else {
...
void Propagator::compute_F_and_G_analytic(std::shared_ptr<State> state, double dt, const Eigen::Vector3d &w_hat,
                                          ...
                                          Eigen::MatrixXd &G) {
  if (state->_options.do_tekf) {
    Eigen::Matrix3d R_itoG = quat_2_Rot(state->_imu->quat()).transpose();
    Eigen::Matrix3d Xi_1 = Xi_sum.block(0, 3, 3, 3);
    Eigen::Matrix3d Xi_2 = Xi_sum.block(0, 6, 3, 3);
    ...
    F.block<3, 3>(0, 9) = -R_itoG * Jl_so3(w_hat * dt) * dt;

    F.block<3, 3>(3, 0) = -skew_x(R_itoG* Xi_2*a_hat);
    F.block<3, 3>(3, 3).setIdentity();
    F.block<3, 3>(3, 6) = Eigen::Matrix3d::Identity() * dt;
    F.block<3, 3>(3, 9) =  R_itoG * Xi_4;
    F.block<3, 3>(3, 12) = -R_itoG * Xi_2;

    F.block<3, 3>(6, 0) = -skew_x(R_itoG* Xi_1*a_hat);
    F.block<3, 3>(6, 6).setIdentity();
    F.block<3, 3>(6, 9) =  R_itoG * Xi_3;
    F.block<3, 3>(6, 12) = -R_itoG * Xi_1;

    F.block<3, 3>(9, 9).setIdentity();
    F.block<3, 3>(12, 12).setIdentity();

    G.block<9, 6>(0, 0) = F.block<9, 6>(0, 9);
    G.block(9, 6, 3, 3) = Eigen::Matrix3d::Identity() * dt;
    G.block(12, 9, 3, 3) = Eigen::Matrix3d::Identity() * dt;
...
void Propagator::compute_F_and_G_discrete(std::shared_ptr<State> state, double dt, const Eigen::Vector3d &w_hat,
  ...
  if (state->_options.do_tekf) {
    ...
    F.block<3, 3>(3, 0) = -0.5 * skew_x(R_itoG * a_hat) * dt * dt;
    F.block<3, 3>(3, 3).setIdentity();
    F.block<3, 3>(3, 6) = Eigen::Matrix3d::Identity() * dt;
    F.block<3, 3>(3, 12) = -0.5 * R_itoG * dt * dt;

    F.block<3, 3>(6, 0) =  -skew_x(R_itoG * a_hat) * dt;
    F.block<3, 3>(6, 6).setIdentity();
    F.block<3, 3>(6, 12) = -R_itoG * dt;
    ...
    // F.block<3, 3>(0, 0).setIdentity();
    // F.block<3, 3>(0, 9) = -R_itoG * Jr_so3(w_hat * dt) * dt;
