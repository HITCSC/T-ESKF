...
void VioManager::initialize_with_gt(Eigen::Matrix<double, 17, 1> imustate) {
  ...
  Cov = Cov*std::pow(0.1,state->_options.init_scale) ;
  ...
  //   Eigen::Matrix<double, 15, 15> T_theta_tekf;
  //   T_theta_tekf.setIdentity();
  //   T_theta_tekf.block<3, 3>(0, 0) = R_itoG;
  //   T_theta_tekf.block<3, 3>(3, 0) = skew_x(p_iinG) * R_itoG;
  //   T_theta_tekf.block<3, 3>(6, 0) = skew_x(v_iinG) * R_itoG;
  //   Cov = T_theta_tekf * Cov * T_theta_tekf.transpose();
  // }
...
bool VioManager::try_to_initialize(const ov_core::CameraData &message) {
  ...
  std::thread thread([&] {
    ...
    if (success) {
      ...
      //   covariance = T_theta_tekf * covariance * T_theta_tekf.transpose();

      //   // covariance = std::pow(0.02, 2) * Eigen::MatrixXd::Identity(15, 15);
      ...
      //   // Eigen::Matrix<double, 15, 15> T_theta_tekf;
      //   // T_theta_tekf.setIdentity();
      //   // T_theta_tekf.block<3, 3>(3, 0) = skew_x(p_iinG);
      //   // T_theta_tekf.block<3, 3>(6, 0) = skew_x(v_iinG);
      //   // covariance = T_theta_tekf * covariance * T_theta_tekf.transpose();
      // }
