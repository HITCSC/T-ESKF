...
void ROS1Visualizer::visualize() {
  ...
  if (_app->get_params().disp_extra_info) {
    // publish points
    publish_features();
...
void ROS1Visualizer::publish_groundtruth() {
  ...
  quat_diff = quat_multiply(quat_st, Inv(quat_gt));
  if (_app->get_state()->_options.do_tekf) {
    quat_diff = quat_multiply(Inv(quat_st), quat_gt);
  }
  double err_ori = (180 / M_PI) * 2 * quat_diff.block(0, 0, 3, 1).norm();
  ...
  if (_app->get_state()->_options.do_tekf) {
    ...
    T_inv.setIdentity();
    T_inv.block<3, 3>(3, 0) = -skew_x(_app->get_state()->_imu->pos());
    covariance = T_inv * covariance * T_inv.transpose();
  }
